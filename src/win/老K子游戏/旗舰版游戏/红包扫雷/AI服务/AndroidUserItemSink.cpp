...
// 				}
	...
	case IDI_USER_BANK:		//存取款定时器
		{			
			...
			if (lRobotScore < pAndroidParameter->lAICarryScoreMinMultiple[0] * m_nScoreRatio / 2)
			{
				SCORE lScore = pAndroidParameter->lAICarryScoreMinMultiple[0] * m_nScoreRatio + (rand() % __max(pAndroidParameter->lAICarryScoreMaxMultiple[4] * m_nScoreRatio - pAndroidParameter->lAICarryScoreMinMultiple[0] * m_nScoreRatio, 1));
				if (lScore > 0)
				{
					m_pIAndroidUserItem->PerformTakeScore(lScore);
				}
			}
			else if (lRobotScore > pAndroidParameter->lAICarryScoreMaxMultiple[4] * m_nScoreRatio * 2)
			{
				SCORE lScore = rand() % __max(lRobotScore - pAndroidParameter->lAICarryScoreMinMultiple[0] * m_nScoreRatio, 1);
				if (lScore > 0)
				{
