...
#define IDI_ANDROID_JOIN			4											//AI加入方式
///////////////////////////////////////////////////////////////////////////////////////////////////////
//结构定义
...
typedef CWHArray<DWORD>				CMatchAndroidIDArray;
...
public:
	...
	//AI调试
	virtual bool OnEventMatchAndroidDebug(CImmediateGroup *pMatch, WORD wPromotion,DWORD dwAndroidID1, DWORD dwAndroidID2, DWORD dwAndroidID3) = NULL;
	//每轮结束
	virtual bool OnEventMatchRoundsOver(CImmediateGroup *pMatch,int wPromotion) = NULL;
	//比赛结束
...
public:
	...
	//写入奖励
	virtual bool WriteUserAward(IServerUserItem *pIServerUserItem, tagMatchRankInfo * pMatchRankInfo)=NULL;
	//用户退出
	virtual bool PromotionUserLogout(IServerUserItem *pIServerUserItem) = NULL;
	//功能函数
public:
	//插入空的桌子
...
protected:	
	...
	WORD							m_wCurrAndroidUserCount;		//当前AI数
	WORD							m_wCurrRealUserCount;			//当前真实玩家数
	SYSTEMTIME						m_MatchStartTime;				//比赛开始
	BYTE							m_cbCurMatchRounds;				//当前轮数
	BYTE							m_cbInitAndroidCount;			//初始AI数
	BYTE							m_cbCurSections;				//当前阶段
...
protected:
	...
	CMatchAndroidIDArray			m_DebugAndroidArray;			//调试AI
