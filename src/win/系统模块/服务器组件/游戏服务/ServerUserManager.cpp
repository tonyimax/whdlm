...
CServerUserItem::CServerUserItem()
{
	...
	memset(m_byGameoverData, 0, sizeof(m_byGameoverData));
	...
	ZeroMemory(m_szGroupID, sizeof(m_szGroupID));
...
//设置挂机状态
VOID CServerUserItem::SetAndroidUserHanging(bool bAndroidUserHanging)
{
	if (bAndroidUserHanging != m_bAndroidUserHanging)
	{
		m_bAndroidUserHanging = bAndroidUserHanging;

		//发送状态
		//ASSERT(m_pIServerUserItemSink != NULL);
		if (m_pIServerUserItemSink != NULL)
		{
			m_pIServerUserItemSink->OnEventAndroidUserHangingNotify(this, m_bAndroidUserHanging);
		}
	}
}

//设置机器等级(携带和离开等级)
VOID CServerUserItem::SetAndroidGrade(WORD wAndroidGrade)
{
	if (wAndroidGrade != m_wAndroidGrade)
	{
		m_wAndroidGrade = wAndroidGrade;
	}
	...
	if (m_pIServerUserItemSink != NULL)
	{
		m_pIServerUserItemSink->OnEventAndroidUserGradeNotify(this, m_wAndroidGrade);
	}
}
...
//视频ID
void	CServerUserItem::SetVideoNumber(CHAR *szVideoNumber)
{
	memcpy(m_szVideoNumber,szVideoNumber,sizeof(m_szVideoNumber));
}
//游戏结束数据
void	CServerUserItem::SetGameoverData(BYTE *byGameoverData)
{
	memcpy(m_byGameoverData, byGameoverData, sizeof(m_byGameoverData));
}
...
//写入积分
bool CServerUserItem::WriteUserScore(SCORE lScore, SCORE lGrade, SCORE lRevenue, SCORE lChipSerialScore, SCORE lIngot, BYTE cbScoreType, DWORD dwPlayTimeCount, DWORD dwWinExperience, DWORD dwGroupID)
{
	//效验状态
	...
	m_RecordInfo.lChipSerialScore += lChipSerialScore;
	...
	m_VariationInfo.lChipSerialScore += lChipSerialScore;
	m_VariationInfo.dwGroupID = dwGroupID;
	//托管积分
	...
	//每日输赢累计
	DWORD dwCurTime = (DWORD)time(0) + 8 * HOUR_SECONDS;	//转换北京时间

	//是否隔天
	if (dwCurTime / (DWORD)DAY_SECONDS != m_dwDayLogonTime / (DWORD)DAY_SECONDS)
	{
		//从当天0点0分0秒开始计算	
		m_dwDayLogonTime = dwCurTime / (DWORD)DAY_SECONDS * (DWORD)DAY_SECONDS;
		m_dwDayPlayTime = 0;
		m_lDayWinLostScore = 0;
	}
	m_lDayWinLostScore += lScore;
...
VOID CServerUserItem::InitUserGameDataMap()
{
	...
	{
		_sntprintf_s(szSubItem,CountArray(szSubItem),TEXT("%s"),SubItemArray.GetAt(i-1));
...
bool CServerUserItem::IsValidGameData(CString strSource, CString strSpliter)
{
	...
	{
		_sntprintf_s(szSubItem,CountArray(szSubItem),TEXT("%s"),SubItemArray.GetAt(i));
...
bool CServerUserManager::InsertUserItem(IServerUserItem * * pIServerUserResult, tagUserInfo & UserInfo,tagUserInfoPlus & UserInfoPlus)
{
	...
	pServerUserItem->m_dwDayLogonTime = pServerUserItem->m_dwLogonTime + 8 * HOUR_SECONDS;	//转换北京时间
	...
	pServerUserItem->m_dwDayPlayTime = UserInfoPlus.dwDayPalyTime;
	pServerUserItem->m_lDayWinLostScore = UserInfoPlus.lDayWinLostScore;
	...
	pServerUserItem->SetUserGroup(UserInfoPlus.szGroupID);
...
//设置俱乐部
VOID CServerUserItem::SetUserGroup(TCHAR *pszGroupID)
{
	//参数校验
	if (m_szGroupID == NULL) return;
	if (CountStringBufferW(pszGroupID) == 0) return;

	ZeroMemory(m_szGroupID, sizeof(m_szGroupID));
	//拷贝数据
	CopyMemory(m_szGroupID, pszGroupID, CountStringBufferW(pszGroupID));

}


//每日时间
DWORD CServerUserItem::GetDayPlayTime()
{
	//游戏时间计算
	DWORD dwCurTime = (DWORD)time(0) + 8 * HOUR_SECONDS;	//转换北京时间
	...
	return m_dwDayPlayTime + ((dwCurTime>m_dwDayLogonTime) ? (dwCurTime - m_dwDayLogonTime) : 0);
}
//////////////////////////////////////////////////////////////////////////////////
