...
std::string makeHandshakeResponse(const char* seckey,std::string protocol) {
    std::string answer("");
...
*/
WsFrameType decodeFrame(const char* inbuf, int inlength, char* outbuf, int* outlen, int* frameSize) {
    ...
    if (length_field <= PAYLOAD_SIZE_BASIC) {
        ...
        uint16_t tmp16 = 0;
        memcpy(&tmp16, inp + 2, 2);
        payload_length = ntoh16(tmp16);
        //payload_length = inp[2] + (inp[3] << 8);
        pos += 2;
    } else if (length_field == 127) { //msglen is 64bit!
        uint64_t tmp64 = 0;
        memcpy(&tmp64, inp + 2, 8);
        payload_length = ntoh64(tmp64);
        //payload_length = inp[2] + (inp[3] << 8);
        pos += 8;
    }
    int frameLength = payload_length + pos;
    if (msg_masked == true) {
        frameLength+=4;
    }
    if (inlength < frameLength) {
        //printf("warn: inlengh<frameLength\n");
        return WS_INCOMPLETE_FRAME;
    } else if (inlength>frameLength) {
        //printf("warn: inlengh>frameLength:%d>%d\n", inlength, frameLength);
    }
    //else if (payload_length >(int)outbuf->size())
    //{
    //	outbuf->resize(payload_length);
    //}

    if (msg_masked&&payload_length>0) {
        mask = *((unsigned int*)(inp + pos));
        pos += 4;
        memcpy(outbuf, (void*)(inp + pos), payload_length);
        // unmask data:
        char* c = outbuf;
...
int encodeFrame(WsFrameType frame_type, const char* msg, int msg_length, char* outbuf, int bufsize) {
    ...
    //fix by xujialong 数据长度检查
    if (size > 0 &&(size+pos)<bufsize) {
