...
LPCTSTR CMatchListControl::GetDescribeString(tagGameMatchOption * pGameMatchOption, WORD wColumnIndex)
{
	...
	//		{
	//			_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("扣除金币"));
	//		}
	//		else if(pGameMatchOption->cbSignupMode==SIGNUP_MODE_MATCH_USER)
	//		{
	//			_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("跳转网站"));
	//		}
...
VOID CRewardListControl::InitListControl()
{
	...
	InsertColumn(nColIndex++,TEXT("奖励钻石"),LVCFMT_LEFT,100);
	InsertColumn(nColIndex++,TEXT("奖励奖券"),LVCFMT_LEFT,100);
...
INT CRewardListControl::SortListItemData(LPARAM lParam1, LPARAM lParam2, WORD wColumnIndex)
{
	...
	case 3:		//奖励奖券
		{
			return pMatchRewardInfo1->dwRewardTicket>pMatchRewardInfo2->dwRewardTicket;
		}
	}
...
LPCTSTR CRewardListControl::GetDescribeString(tagMatchRewardInfo * pMatchRewardInfo, WORD wColumnIndex)
{
	...
	case 0:		//比赛名次
		{
			_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("第%02d名"),pMatchRewardInfo->wRankID);

			return szDescribe;
		}
	case 1:		//金币奖励
		{
			_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("%I64d"),pMatchRewardInfo->lRewardGold);

			return szDescribe;
		}
	case 2:		//元宝奖励
		{
			_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("%I64d"),pMatchRewardInfo->lRewardIngot);

			return szDescribe;
		}
	case 3:		//奖券奖励
		{
			_sntprintf_s(szDescribe, CountArray(szDescribe), TEXT("%d"), pMatchRewardInfo->dwRewardTicket);
