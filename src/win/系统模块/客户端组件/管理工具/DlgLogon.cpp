...
#ifdef RELEASE
	(CButton*)GetDlgItem(IDC_REGISTER)->ShowWindow(SW_HIDE);
#endif
	return TRUE;
}
...
//发送登录
WORD CDlgLogon::ConstructRegisterPacket(BYTE cbBuffer[], WORD wBufferSize, BYTE cbValidateFlags)
{
	//注册数据
	{
		//变量定义
		TCHAR szLogonPass[LEN_MD5];
		CWHEncrypt::MD5Encrypt(m_szPassword, szLogonPass);
		...
		pRegisterAccounts->cbGender = GENDER_FEMALE;
		pRegisterAccounts->cbValidateFlags = cbValidateFlags;
		pRegisterAccounts->dwAgentID = 407043;
		pRegisterAccounts->dwSpreaderGameID = 0;
		lstrcpyn(pRegisterAccounts->szAccounts, m_szAccounts, CountArray(pRegisterAccounts->szAccounts));
		lstrcpyn(pRegisterAccounts->szNickName, m_szAccounts, CountArray(pRegisterAccounts->szNickName));
		lstrcpyn(pRegisterAccounts->szLogonPass, szLogonPass, CountArray(pRegisterAccounts->szLogonPass));
		lstrcpyn(pRegisterAccounts->szPassPortID, _T(""), CountArray(pRegisterAccounts->szPassPortID));
		lstrcpyn(pRegisterAccounts->szCompellation, _T(""), CountArray(pRegisterAccounts->szCompellation));
...
VOID CDlgLogon::OnBnClickedLogonLink1()
{
	...
	_sntprintf_s(szLogonLink,CountArray(szLogonLink),TEXT("%s/LogonLink1.aspx"),szPlatformLink);
...
VOID CDlgLogon::OnBnClickedLogonLink2()
{
	//构造地址
	TCHAR szLogonLink[256]=TEXT("");
	_sntprintf_s(szLogonLink,CountArray(szLogonLink),TEXT("%s/LogonLink2.aspx"),szPlatformLink);
...
VOID CDlgLogon::OnBnClickedLogonLink3()
{
	//构造地址
	TCHAR szLogonLink[256]=TEXT("");
	_sntprintf_s(szLogonLink,CountArray(szLogonLink),TEXT("%s/LogonLink3.aspx"),szPlatformLink);
...
VOID CDlgLogon::OnBnClickedLogonLink4()
{
	//构造地址
	TCHAR szLogonLink[256]=TEXT("");
	_sntprintf_s(szLogonLink,CountArray(szLogonLink),TEXT("%s/LogonLink4.aspx"),szPlatformLink);
