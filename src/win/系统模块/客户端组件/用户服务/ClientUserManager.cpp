...
//增加用户
IClientUserItem * CPlazaUserManager::ActiveUserItem(tagUserInfo & UserInfo, tagCustomFaceInfo & CustomFaceInfo, bool bIsAndroid, SCORE lFScore)
{
	//变量定义
	...
	pClientUserItem->m_lFirstScore = lFScore;
...
bool CPlazaUserManager::DeleteUserItem(IClientUserItem * pIClientUserItem)
{
	...
	for (INT_PTR i=0;i<m_UserItemActive.GetCount();i++)
	{
		...
		{
			if (pUserItemActive->IsAndroidUserHanging())
			{
				pUserItemActive->SetAndroidUserHanging(false);
			}

			//重设登录时间
			pUserItemActive->SetLogonTime(0);

			//重设机器等级
			pUserItemActive->SetAndroidGrade(AI_CARRY_GRADE_NONE | AI_LEAVE_GRADE_NONE);
...
//增加用户
IClientUserItem * CGameUserManager::ActiveUserItem(tagUserInfo & UserInfo, tagCustomFaceInfo & CustomFaceInfo, BYTE cbCompanion, LPCTSTR pszUserNote, bool bIsAndroid, SCORE lFScore)
{
	//效验状态
