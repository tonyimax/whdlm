...
BEGIN
	...
	EXEC ('SELECT GroupID,Ingot,ConsumeIngot,BattleCreate,Score FROM IMGroupWealth(NOLOCK) WHERE GroupID IN(SELECT GroupID FROM #Tb_Group_Cache)')
	EXEC ('SELECT GroupID,AwardPoint FROM IMGroupMember(NOLOCK) WHERE MemberRight=1 AND GroupID IN(SELECT GroupID FROM #Tb_Group_Cache)')
