...
BEGIN
	...
	BEGIN
		...
		RETURN 1002
	END
	select @UserID=userid from WHQJAccountsDB.dbo.AccountsInfo where GameID=@strGameid
	IF @UserID IS NULL
	BEGIN
		SET @strErrorDescribe=N'抱歉，您的帐号不存在！'
		RETURN 1002
	END
	if not EXISTS (SELECT AgentID FROM AgentInfo WITH(NOLOCK) WHERE  [Password] = @strPassword)
	 begin
	   SET @strErrorDescribe=N'抱歉，您的密码错误！'
		RETURN 1002
	 end

  -- 查询代理
  SELECT @AgentID = AgentID,@UserID = UserID,@AgentNullity=Nullity FROM AgentInfo WITH(NOLOCK) WHERE  [Password] = @strPassword and UserID=@UserID
  ...
  IF @UserID IS NULL
	...
	-- 查询用户
	SELECT @Nullity=Nullity, @StunDown=StunDown
