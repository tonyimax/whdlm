...
BEGIN
	...
	BEGIN
		...
		SELECT @BeforeScore = Score,@BeforeInsure = InsureScore FROM GameScoreInfo(NOLOCK) WHERE UserID = @UserID
		   set @Score=@Score+@OtherPresent
		-- 更新用户金币，只增加额外部分
		UPDATE GameScoreInfo SET Score = Score + @Score WHERE UserID = @UserID
		...
		VALUES(dbo.WF_GetSerialNumber(), 0, @UserID, 12, @BeforeScore, @BeforeInsure, @Score, @OrderAddress, @DateTime)
	END
	...
	BEGIN
		...
		SELECT @BeforeScore = Diamond FROM UserCurrency(NOLOCK) WHERE UserID = @UserID
		 set @Score=@Score+@OtherPresent
		-- 更新用户流水，只增加额外部分
		UPDATE UserCurrency SET Diamond = Diamond + @Score WHERE UserID = @UserID
		...
		VALUES(dbo.WF_GetSerialNumber(),0,@UserID,14,@BeforeScore,@Score,@OrderAddress,@DateTime)
	END

	-- 处理完成后
	-- 更新订单表 BeforeScore 更新ScoreType 分别插入原值 0:游戏币 1：钻石 2.。。
	UPDATE OnLinePayOrder SET OrderStatus = 2,PayDate=GETDATE()  WHERE OrderID = @strOrdersID AND OrderStatus = 1
