...
BEGIN
	...
	-- 插入记录
	INSERT RecordDrawScore(DrawID,UserID,ChairID,Score,Grade,Revenue,PlayTimeCount,DBQuestID,InoutIndex)
	VALUES (@dwDrawID,@dwUserID,@wChairID,@lScore,@lGrade,@lRevenue,@dwPlayTimeCount,@dwDBQuestID,@dwInoutIndex)
...
BEGIN
	...
	INSERT INTO WHQJGroupDB..RecordGroupRoomInfo(GroupID,RoomID,PlayMode,KindID,KindName,DrawCount,BigWinnerID,BigWinnerName,MinTableScore,StartTime,EndTime)
					VALUES(@dwGroupID,@dwRoomID,@cbPlayMode,@wKindID,@szServerName,@wDrawCount,@dwWinnerGameID,@szWinnerName,@lMinEnterScore,@SystemTimeStart,@SystemTimeConclude)
...
BEGIN
	...
	BEGIN
		...
		SET DrawCount=DrawCount+1,BigWinnerCount=BigWinnerCount+@cbBigWinner, ScoreCount=ScoreCount+@lScore, RevenueCount=RevenueCount+@lRevenue ,CurScore=@CurUserScore-@lRevenue 
	...
	BEGIN
		...
		VALUES(@dateID,@dwGroupID,@dwUserID,@szUserName,@CurUserScore-@lRevenue,0,1,@cbBigWinner,@lScore,@lRevenue,0,0,@TimeEnd)
	END
	...
	--小队信息统计
	DECLARE @ContributionRate SMALLINT
	SELECT @ContributionRate=OptionValue FROM   WHQJGroupDB.dbo.IMGroupOption WHERE OptionName=N'ContributionRate' 
	IF @ContributionRate IS NULL OR @ContributionRate<0 OR @ContributionRate>100
	...
	BEGIN
		...
		SET @TempRevenue=@lRevenue*(100-@ContributionRate)/100
		--盟主得到的返利
		...
		BEGIN
			...
			BEGIN
				...
				set DrawCount=DrawCount+1,BigWinnerCount=BigWinnerCount+@cbBigWinner,ScoreCount=ScoreCount+@lScore,RevenueCount=RevenueCount+@lRevenue,ContributionValue=ContributionValue+@TempRevenue
			...
			BEGIN
				...
				values(@dateID,@dwGroupID,@dwTeamerID,1,@cbBigWinner,@lScore,@lRevenue,@TempRevenue,0)
			END
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				update   WHQJGroupDB.dbo.RecordTeamDayInfo 
				set DrawCount=DrawCount+1,BigWinnerCount=BigWinnerCount+@cbBigWinner,ScoreCount=ScoreCount+@lScore,RevenueCount=RevenueCount+@lRevenue
				where DateID=@dateID AND CaptainID=@dwUserID AND GroupID=@dwGroupID
			END
			ELSE
			BEGIN
				insert   WHQJGroupDB.dbo.RecordTeamDayInfo (DateID,GroupID,CaptainID,DrawCount,BigWinnerCount,ScoreCount,RevenueCount,ContributionValue,Flag)
				values(@dateID,@dwGroupID,@dwUserID,1,@cbBigWinner,@lScore,@lRevenue,0,0)
			END
		END
		...
		BEGIN
			...
			ELSE
			BEGIN
				insert   WHQJGroupDB.dbo.RecordTeamDayInfo (DateID,GroupID,CaptainID,DrawCount,BigWinnerCount,ScoreCount,RevenueCount,Flag)
				values(@dateID,@dwGroupID,@dwCaptainID,1,@cbBigWinner,@lScore,@lRevenue,0)
			END
		...
		BEGIN
			...
			SET @TempRevenue=@lRevenue*(100-@ContributionRate-@DownPoint)/100
			IF @TempRevenue>0
			BEGIN
				--上级得到的返利
				UPDATE   WHQJGroupDB.dbo.IMGroupMember SET AwardScore=AwardScore+@TempRevenue WHERE GroupID=@dwGroupID AND MemberRight=1
